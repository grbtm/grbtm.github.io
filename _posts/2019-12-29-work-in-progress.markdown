...
I had two stumbling blocks in getting started:
1. In order to produce meaningful results, training the `doc2vec` models demands a sufficiently large and also
appropriate (with respect to the documents that will be queried in production) corpus of documents.
As already discussed in other places (add links), it either doesn't make sense to use pre-trained
...
layer (CHECK). For a nice introductory tutorial on `doc2vec`, see the already stated [documentation][gensim-doc2vec-tutorial].
...
The first basic assessment reveals that:
  ...
  - the worst-performing model with self-reference percentage of 82.7% is PV-DM with averaging of context word vectors + higher starting alpha + large context window
